...
(defn- ^number hash-arr [arr]
  (loop [n 0
         hash-code 1]
    (if (< n (alength arr))
      (recur (inc n) (bit-or (+ (imul 31 hash-code) (hash (aget arr n))) 0))
...
(deftype CollRelation [symbols offsets arity coll]
  IReduce
  (-reduce [_ f init]
    (reduce f init coll))
  ...
  IRelation
  (-getter [_ symbol]
    (let [idx (get (zipmap symbols offsets) symbol)]
      (fn [tuple]
        (nth tuple idx))))
  (-indexes [_ syms]
    (mapa (zipmap symbols offsets) syms))
  (-copy-tuple [_ tuple idxs target offset]
    (dotimes [i (alength idxs)]
  ...
  (-pr-writer [_ writer opts]
    ...
    (pr-writer {:symbols symbols, :offsets offsets, :arity arity, :coll coll} writer opts)))

(defn coll-rel [symbols coll]
  (let [s->i    (zipmap symbols (range))
        symbols (remove #{'_} symbols)
        indexes (mapa s->i symbols)]
    (CollRelation. symbols indexes (count symbols) coll)))
...
(deftype ProdRelation [symbols arity rel1 rel2]  
  ...
  (-indexes [_ syms]
    (let [[syms1 syms2] (split-with (set (.-symbols rel1)) syms)]
      #js [(-indexes rel1 syms1) (-indexes rel2 syms2)]))
  (-copy-tuple [_ tuple idxs target offset]
...
(defn hash-join [rel1 hash1 join-syms rel2]
  (let [syms1       (.-symbols rel1)
        ...
        coll        (reduce ;; iterate over rel2
                      (fn [acc t2]
                        (let [tuples1 (-lookup hash1 (key-fn2 t2) nil)]
                          (if (nil? tuples1)
                            acc
                            (array-reduce ;; iterate over corresponding rel1 tuples
                              tuples1
                              (fn [acc t1]
                                (conj! acc (join-tuples rel1 t1 idxs1
                                                        rel2 t2 idxs2
...
(defn- plain-tuple-binding? [binding]
  (and (instance? dp/BindTuple binding)
       (every? #(or (instance? dp/BindScalar %)
                    (instance? dp/BindIgnore %))
               (.-bindings binding))))

(defn- bind-tuples [bindings coll]
  (coll-rel (mapv #(if (instance? dp/BindScalar %)
                     (.. % -variable -symbol)
                     '_)
              bindings)
    coll))
...
(extend-protocol IBinding
  ...
  dp/BindScalar
  (in->rel [binding value]
    (array-rel [(.. binding -variable -symbol)] [#js [value]]))
  ...
  (in->rel [binding coll]
    (let [inner-binding (.-binding binding)]
      (cond
        ...
        ;; [?x ...] type of binding
        (instance? dp/BindScalar inner-binding)
...
(defn get-source [context source]
  (let [symbol (cond
                 (instance? dp/SrcVar source)     (.-symbol source)
                 (instance? dp/DefaultSrc source) (:default-source-symbol context)
...
(defn- bind-pattern [pattern coll]
  (coll-rel (mapv #(if (instance? dp/Variable %)
                     (.-symbol %)
                     '_)
              pattern)
    coll))
...
(defn -resolve-pattern-coll [coll clause]
  ...
  (let [pattern (.-pattern clause)
        idxs    (->> (map #(when (instance? dp/Constant %1) [%2 (.-value %1)]) pattern (range))
                     (remove nil?)
                     (into {}))
  ...
  (-resolve-clause [clause context]
    (let [source (get-source context (.-source clause))]
      (if (satisfies? dc/ISearch source)
        false
;;         (-resolve-pattern-db source (.-pattern clause))
        (-resolve-pattern-coll source clause)))))

(defn resolve-clause-new [context clause]
  (measure (-resolve-clause clause context)
...
(defn resolve-clause-related [context clause clause-syms related-rels]
  (measure
    (let [related   (reduce product related-rels)
          ...
          rel       (measure (-resolve-clause clause context) ;; TODO use hash
                             "-resolve-clause to" (count %) "tuples")
          ;; TODO check size, avoid hash-join on 1 or 0 
...
(defn resolve-clause [context clause]
  (measure
    (let [clause-syms  (measure (into #{} (map :symbol) (dp/collect #(instance? dp/Variable %) clause #{})) "clause-syms")
          ...
          clause*      (substitute-constants clause context)
          ;; TODO pre-check zeroes
          new-rel      (cond
