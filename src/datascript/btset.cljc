...
(defn splice [arr splice-from splice-to xs]
  (cut-n-splice arr 0 (shim/alength arr) splice-from splice-to xs))
...
(defn arr-map-inplace [f arr]
  (let [len (shim/alength arr)]
    (loop [i 0]
      (when (< i len)
        (shim/aset arr i (f (shim/aget arr i)))
        (recur (inc i))))
    arr))
...
(defn- sorted-arr-distinct
  ...
  (if (sorted-arr-distinct? arr cmp)
    arr
    (let [al (shim/alength arr)]
      (loop [acc (transient [(shim/aget arr 0)])
             i   1
             p   (shim/aget arr 0)]
        (if (>= i al)
  ...
  (cond
    ...
    (and left (or (nil? right)
      ...
      (let [nodes (node-merge-n-split left node)]
        (return-array (shim/aget nodes 0) (shim/aget nodes 1) right))

    ;; right has fewer nodes, redestribute with it
    :else
      (let [nodes (node-merge-n-split node right)]
        (return-array left (shim/aget nodes 0) (shim/aget nodes 1)))))
...
(deftype Node [keys pointers]
  ...
  (node-merge [_ next]
    (Node. (shim/aconcat keys (.-keys ^Node next))
...
;;   (toString [_] (pr-str* (vec keys)))
  ...
  (node-merge [_ next]
    (Leaf. (shim/aconcat keys (.-keys ^Leaf next))))
          ...
          (let [middle (shim/half (inc keys-l))]
            (if (> idx middle)
              ...
              (shim/array
                ...
                (Leaf. (cut-n-splice keys middle keys-l idx idx (shim/array key))))
              ;; new key goes to the first half
              (shim/array
                (Leaf. (cut-n-splice keys 0 middle idx idx (shim/array key)))
...
(defn -next-path ^long [node ^long path ^long level]
  (let [idx (path-get path level)]
    (if (pos? level)
      ;; inner node
      (let [sub-path (-next-path (shim/aget (.-pointers ^Node node) idx) path (- level level-shift))]
        (if (== -1 sub-path)
          ;; nested node overflow
          (if (< (inc idx) (shim/alength (.-pointers ^Node node)))
      ...
      ;; leaf
      (if (< (inc idx) (shim/alength (.-keys ^Leaf node)))
        ;; advance leaf idx
  ...
  (loop [node  node
    ...
    (if (pos? level)
      ...
      (recur (alast (.-pointers ^Node node))
             (path-set path level (dec (shim/alength (.-pointers ^Node node))))
             (- level level-shift))
      ;; leaf
  ...
  (let [idx (path-get path level)]
    (if (pos? level)
      ...
      (let [sub-level (- level level-shift)
            sub-path  (-prev-path (shim/aget (.-pointers ^Node node) idx) path sub-level)]
        (if (== -1 sub-path)
          ;; nested node overflow
          (if (>= (dec idx) 0)
            ;; advance current node idx, reset subsequent indexes
            (let [idx      (dec idx)
                  sub-path (-rpath (shim/aget (.-pointers ^Node node) idx) sub-level)]
              (path-set sub-path level idx))
            ;; current node overflow
            -1)
          ;; keep current idx
    ...
    (let [set   (.-set iter)
      ...
      (loop [left (.-left iter)
        ...
        (cond
          ...
          (nil? keys)    acc
          :else
            (let [new-acc (f acc (shim/aget keys idx))]
              (if (< (inc idx) (shim/alength keys))
                ;; can use cached array to move forward
                (if (< (inc left) right)
...
(defn riter-first [^ReverseIter riter]
  (when (.-keys riter)
    (shim/aget (.-keys riter) (.-idx riter))))
...
(defn -distance [node ^long left ^long right ^long level]
  (let [idx-l (path-get left level)
    ...
    (if (pos? level)
      ;; inner node
      (if (== idx-l idx-r)
        (-distance (shim/aget (.-pointers ^Node node) idx-l) left right (- level level-shift))
        (loop [level level
               res   (- idx-r idx-l)]
          (if (== 0 level)
            res
  ...
  (let [leafs (->> arr
    ...
    (loop [current-level leafs
      ...
      (case (count current-level)
        0 (BTSet. (Leaf. (shim/array)) 0 0 cmp nil uninitialized-hash)
        1 (BTSet. (first current-level) shift (shim/alength arr) cmp nil uninitialized-hash)
        (recur (->> current-level
                    (arr-partition-approx min-len max-len)
...
(defn btset-by
  ([cmp] (BTSet. (Leaf. (shim/array)) 0 0 cmp nil uninitialized-hash))
  ([cmp & keys]
