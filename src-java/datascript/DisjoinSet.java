...
/**
* AtomicBooleanSet + disjoin
*/

public class DisjoinSet implements IPersistentSet {
  ...
  public static void setMaxLen(int maxLen) {
    DisjoinSet.maxLen = maxLen;
    DisjoinSet.minLen = maxLen >>> 1;
  }

  public DisjoinSet() { this(Comparator.naturalOrder()); }
  public DisjoinSet(int maxLen) {
    this();
    setMaxLen(maxLen);
  }

  public DisjoinSet(Comparator cmp) {
  ...
  DisjoinSet(Leaf root, int size, int depth, AtomicBoolean edit) {
    this.root = root;
    this.size = size;
    this.depth = depth;
    this.setEdit = edit;
  ...
  public DisjoinSet asTransient() {
    return new DisjoinSet(root, size, depth, new AtomicBoolean(true));
  }

  public DisjoinSet persistent() {
    setEdit.setPlain(false);
    return this;
  }

  public DisjoinSet add(Object key) {
    ...
    if (1 == nodes.length)
      return new DisjoinSet(nodes[0], size+1, depth, setEdit);
    
    Object keys[] = new Object[] { nodes[0].maxKey(), nodes[1].maxKey() };
    Leaf newRoot = new Node(keys, nodes, 2, root.cmp, setEdit);
    return new DisjoinSet(newRoot, size+1, depth+1, setEdit);
  }

  public DisjoinSet remove(Object key) {
    ...
    return new DisjoinSet(newRoot, size-1, depth, setEdit);
  }
  ...
  class Leaf {
    ...
    Leaf newLeaf(int len, AtomicBoolean edit) {
      if (edit.getPlain())
        return new Leaf(new Object[Math.min(DisjoinSet.maxLen, len + DisjoinSet.extraLen)], len, cmp, edit);
      else
    ...
    Leaf[] remove(Object key, Leaf left, Leaf right, AtomicBoolean edit) {
      ...
      // nothing to merge
      if (newLen >= DisjoinSet.minLen || (left == null && right == null)) {
      ...
      // can join with left
      if (left != null && left.len + newLen <= DisjoinSet.maxLen) {
      ...
      // can join with right
      if (right != null && newLen + right.len <= DisjoinSet.maxLen) {
