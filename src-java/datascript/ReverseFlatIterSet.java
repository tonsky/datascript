...
public class ReverseFlatIterSet implements ISortedSet {
  ...
  public ReverseFlatIterSet() { this(Comparator.naturalOrder()); }
  
  public ReverseFlatIterSet(int maxLen) { 
    this(Comparator.naturalOrder());
    this.maxLen = maxLen;
    this.minLen = maxLen / 2;
  }

  public ReverseFlatIterSet(Comparator cmp) {
    this.cmp = cmp;
  ...
  ReverseFlatIterSet(Comparator cmp, Leaf root, int size, int depth, AtomicReference<Thread> edit, int minLen, int maxLen) {
    this.cmp = cmp;
    this.root = root;
    this.size = size;
    this.depth = depth;
  ...
  public ReverseFlatIterSet asTransient() {
    return new ReverseFlatIterSet(cmp, root, size, depth, new AtomicReference<Thread>(Thread.currentThread()), minLen, maxLen);
  }

  public ReverseFlatIterSet persistent() {
    edit.set(null);
    return this;
  }
  ...
  public ReverseFlatIterSet with(Object key) {
    ...
    if (0 == nodes.length)
      return new ReverseFlatIterSet(cmp, root, size+1, depth, edit, minLen, maxLen);

    if (1 == nodes.length)
      return new ReverseFlatIterSet(cmp, nodes[0], size+1, depth, edit, minLen, maxLen);
    
    Object keys[] = new Object[]{nodes[0].maxKey(), nodes[1].maxKey()};
    Leaf newRoot = new Node(keys, nodes, 2, edit);
    return new ReverseFlatIterSet(cmp, newRoot, size+1, depth+1, edit, minLen, maxLen);
  }
  ...
  class Iter implements Iterator {
    int[]  indexes; // root at maxIdx .. leaf at 0
    Leaf[] leaves;
    int maxIdx = depth-1;
    boolean over;
    ...
    void advance() {
      ...
      if (indexes[0]+1 < leaves[0].len) {
        // System.out.println("Fast path " + indexes[0] + " " + leaves[0].len);
        indexes[0]++;
        return;
      }
      ...
      for (int d = 1; d <= maxIdx; ++d) {
        if (indexes[d]+1 < leaves[d].len) {
          // System.out.println("Overflow path at " + d + " with " + indexes[d] + " " + leaves[d].len);
          indexes[d]++;
