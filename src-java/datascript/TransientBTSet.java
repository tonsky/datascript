...
public class TransientBTSet implements IPersistentSet {
  ...
  public TransientBTSet() { this(Comparator.naturalOrder()); }
  
  public TransientBTSet(int maxLen) { 
    this(Comparator.naturalOrder());
    this.maxLen = maxLen;
    this.minLen = maxLen / 2;
  }

  public TransientBTSet(Comparator cmp) {
    this.cmp = cmp;
    edit = new AtomicReference<Thread>(null);
    root = new Node(new Object[]{}, null, 0);
  }

  TransientBTSet(Comparator cmp, Node root, AtomicReference<Thread> edit, int minLen, int maxLen) {
    this.cmp = cmp;
    this.root = root;
    this.edit = edit;
    this.minLen = minLen;
  ...
  class Node {
    ...
    Node(Object[] keys, Node[] children, int len) {
      ...
      this.len = len;
      this.edit = TransientBTSet.this.edit;
    }
  ...
  public TransientBTSet asTransient() {
    return new TransientBTSet(cmp, root, new AtomicReference<Thread>(Thread.currentThread()), minLen, maxLen);
  }

  public TransientBTSet persistent() {
    edit.set(null);
    return this;
  }
  ...
  public TransientBTSet add(Object key) {
    ...
    if (null == nodes)
      return this;
    if (1 == nodes.length)
      return new TransientBTSet(cmp, nodes[0], edit, minLen, maxLen);
    else {
      Object keys[] = new Object[]{nodes[0].maxKey(), nodes[1].maxKey()};
      Node newRoot = new Node(keys, nodes, 2);
      return new TransientBTSet(cmp, newRoot, edit, minLen, maxLen);
    }
  }
