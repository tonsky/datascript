...
public class EarlyExitSet implements IPersistentSet {
  ...
  public EarlyExitSet() { this(Comparator.naturalOrder()); }
  
  public EarlyExitSet(int maxLen) { 
    this(Comparator.naturalOrder());
    this.maxLen = maxLen;
    this.minLen = maxLen / 2;
  }

  public EarlyExitSet(Comparator cmp) {
    this.cmp = cmp;
  ...
  EarlyExitSet(Comparator cmp, Leaf root, int size, AtomicReference<Thread> edit, int minLen, int maxLen) {
    this.cmp = cmp;
    this.root = root;
    this.size = size;
    this.edit = edit;
  ...
  public EarlyExitSet asTransient() {
    return new EarlyExitSet(cmp, root, size, new AtomicReference<Thread>(Thread.currentThread()), minLen, maxLen);
  }

  public EarlyExitSet persistent() {
    edit.set(null);
    return this;
  }
  ...
  public EarlyExitSet add(Object key) {
    ...
    if (0 == nodes.length)
      return new EarlyExitSet(cmp, root, size+1, edit, minLen, maxLen);

    if (1 == nodes.length)
      return new EarlyExitSet(cmp, nodes[0], size+1, edit, minLen, maxLen);
    
    Object keys[] = new Object[]{nodes[0].maxKey(), nodes[1].maxKey()};
    Leaf newRoot = new Node(keys, nodes, 2, edit);
    return new EarlyExitSet(cmp, newRoot, size+1, edit, minLen, maxLen);
  }
